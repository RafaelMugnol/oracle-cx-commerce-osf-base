...
import css from '@oracle-cx-commerce/react-components/page-loader/styles.css';
import SpinnerContent from '@oracle-cx-commerce/react-components/page-loader/spinner-content';
import {PAGE_LOADER} from '@oracle-cx-commerce/react-components/config';
...
const PageLoader = props => {
  const {show = false, className = '', children = ''} = props;
  const {USE_SPINNER_ALWAYS} = PAGE_LOADER;

  return (
    <Styled id="PageLoader" css={css}>
      <div className={objToClassName({PageLoader: true, [className]: true, Dismiss: !show})}>
        {USE_SPINNER_ALWAYS ? <SpinnerContent /> : children}
      </div>
    </Styled>
  );
};
