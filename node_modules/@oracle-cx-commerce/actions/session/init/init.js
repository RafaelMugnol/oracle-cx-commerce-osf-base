...
import {all, select, spawn} from 'redux-saga/effects';
...
export function* initSaga(action) {
  ...
  if (hasSAMLResponse(state)) {
    ...
    const payload = {grantType: 'saml_credentials', samlResponse: getSessionContext(state).samlResponse, relayState};
    const newAction = {...action, endpointId: 'login', payload};

    return yield loginSaga(newAction);
  }
...
export default {
  *saga() {
    yield all([takeOnce('init', initSaga)]);
  }
};
