...
import css from '@oracle-cx-commerce/react-widgets/profile/profile-delete-saved-card/styles.css';
import {getPageData} from '@oracle-cx-commerce/react-widgets/profile/profile-delete-saved-card/selectors';
...
const ProfileDeleteSavedCard = props => {
   ...
   */
  const onConfirmProfileDeleteSavedCard = async (savedCardId, savedCardNumber) => {
    setInProgress(true);
    const response = await action('removeSavedCardFromProfile', {savedCardId});
    if (response.ok) {
      action('notifyClearAll');
      action('notify', {level: INFO, message: t(alertCardDeleted, {cardNumber: savedCardNumber})});
      setInProgress(false);
      const profileSavedCards = getSavedCardsForProfile(getState());
      if (Object.keys(profileSavedCards.savedCardsMap || {}).length) gotoProfileSavedCardsPage();
      else gotoProfilPage();
    } else {
        ...
        <div className="ProfileDeleteSavedCard">
            ...
            message={
              <div className="ProfileDeleteSavedCard__WarnMessageContainer">
                <div className="ProfileDeleteSavedCard__WarnMessage">
                  {t(textCardRemove, {cardtype: cardName, cardnumber: savedCardNumber})}
                </div>
          ...
          <div className="ProfileDeleteSavedCard__ButtonContainer">
            <button
              ...
              onClick={() => onConfirmProfileDeleteSavedCard(savedCardId, savedCardNumber)}
              disabled={inProgress}
            >
              {actionConfirm}
            </button>
