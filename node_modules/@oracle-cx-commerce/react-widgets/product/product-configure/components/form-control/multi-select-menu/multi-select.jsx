...
const MultiSelect = ({
  ...
  useEffect(() => {
    if (inputRef && inputRef.current) {
      for (let i = 0; i < inputRef.current.options.length; i++) {
        if (defaultValue.includes(inputRef.current.options[i].value)) {
  ...
  const handleMultiSelectMenuChange = changeEvent => {
    const {options} = changeEvent.target; // use changeEvent.target.selectedOptions?
    const selectedOptions = [];

    for (let i = 0; i < options.length; i++) {
      if (options[i].selected) {
        selectedOptions.push({value: options[i].value, displayValue: options[i].textContent});
      }
    }
