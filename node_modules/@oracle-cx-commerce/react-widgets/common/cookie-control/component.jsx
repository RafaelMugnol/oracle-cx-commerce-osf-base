...
import {createCookie, removeCookie} from '@oracle-cx-commerce/react-widgets/common/cookie-control/utils';
...
import css from '@oracle-cx-commerce/react-widgets/common/cookie-control/styles.css';
import {getCookie} from '@oracle-cx-commerce/utils/node';
import {necessaryCookies} from '@oracle-cx-commerce/react-widgets/common/cookie-control/config';

/**
 * Widget to take GDPR cookie consent from the user.
 */
const CookieControl = props => {
  const {
  ...
  return (
    <Styled id="CookieControl" css={css}>
      {show && (
        <div className="CookieControl">
          <div className="CookieControl__Heading">{headingGDPRCookieControlTitle}</div>
          <div className="CookieControl__Message">
            {`${textCookieControlMessage1} ${textCookieControlMessage2}`}
            <div>
              {cookiePolicyURL ? (
                <>
                  {textAgreeToUseCookie}
                  <Link className="CookieControl__CookiePolicyLink" href={cookiePolicyURL}>
                    {textCookiePolicyHyperText}
                  </Link>
                </>
              ) : (
          ...
          <div className="CookieControl__Buttons">
            <button type="button" onClick={onConsentAccept} className="CookieControl__AcceptButton">
              {labelAcceptButton}
            </button>
            <button type="button" onClick={onConsentRevoke} className="secondary CookieControl__DeclineButton">
              {labelOptOutButton}
            </button>
          </div>
        </div>
