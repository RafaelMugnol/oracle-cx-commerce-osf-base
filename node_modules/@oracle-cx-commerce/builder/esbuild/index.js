...
const sassInJs = require('./plugins/sass-in-js');
const cssInJs = require('./plugins/css-in-js');
const externalize = require('./plugins/externalize');
...
function createBuilder({
  ...
  const occConfig = {
    ...read(absWorkingDir, 'occ.config.js'),
    ...read(absWorkingDir, 'package.json')
  };

  const {external = [], loader = {}, plugins = [], sass = {}, css = {}} = occConfig;
  ...
  const clientOptions = {
    ...
    outdir: `dist/esm/${min}`,
    target: ['chrome100', 'firefox91', 'safari15'],
    format: 'esm',
    splitting: true,
    external: [],
    plugins: [alias(), externalize(), sassInJs(sass), cssInJs(css), ...plugins]
  };
