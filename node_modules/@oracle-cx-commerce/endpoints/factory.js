...
export {getBodyAsJson} from '@oracle-cx-commerce/endpoints/utils';
...
export const createEndpoint = createEndpointFactory({
  ...
  getHeaders({state, location}) {
    ...
    const {OCStateData, token, tokenType = 'Bearer'} = getSessionContext(state);
    ...
    // to remove the baseURL if it's something other than an empty string. Note that baseUrl is not the same
    // as the baseURI. When baseURI is '/', the baseUrl will be '', It won't be empty when the application server
    // may be using as a routing path (e.g. /occ-live). In this case we remove it from the calculated header
    // so site resolution will work properly.
    //
    ...
    if (priceListGroup) {
      headers['X-CCPriceListGroup'] = priceListGroup;
    }
