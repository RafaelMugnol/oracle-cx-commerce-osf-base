...
const transformPage = (data, response, state) => {
  ...
  if (response.status === 401) {
    delta.clientRepository.context.session = {token: null, tokenType: null};
  }

  if (data.context.pageType !== 'category' && data.context.pageType !== 'search') {
    // Clear (stale) search context when not search/category page
    delta.clientRepository.context.request.search = null;
  }
