...
import {getPage, getSessionContext} from '@oracle-cx-commerce/commerce-utils/selector';
...
export default ({subscribeDispatch}) => {
  return subscribeDispatch((action, state) => {
    ...
    } else if (type === 'verifyResolved' && action?.payload?.status === 401) {
      const {secured} = getPage(state);

      if (secured) {
        // Force refresh of secured pages, e.g. /profile so that the login page is displayed
