  ...
  test('test that each cli command is defined', () => {
    const expectedCommands = [
      ...
      },
      {
        name: 'deploy',
        // eslint-disable-next-line prettier/prettier
        options: ['-V, --version', '-v, --verbose', '--no-verbose', '-k, --appKey [key]', '-s, --appServer <url>', '-a, --appServerAdmin <url>', '-e, --serverEnv <env>', '--deployType <type>', '--ignore <glob>', '--zipFile [zip file location]', '--include <glob>','--localDevAppName <appName>', '--no-localDevAppName', '--no-validation', '--replaceApp <appName>', '-t, --tag <value>', '-p, --publish', '--publishAll [confirm]', '--no-build', '-r, --reset [confirm]', '-l, --list', '--debugMode', '--no-debugMode' , '--backup', '--no-backup' ]
      },
      {
        name: 'redeploy',
        // eslint-disable-next-line prettier/prettier
    ...
    expect(program.commands.length).toEqual(28);
