...
const endpointConfigState = {
  configRepository: {
    endpoints: {
      'listAddresses': {
        url: 'aosfpc1dev-store.occa.us-ashburn-1.ocs.oc-test.com:443/ccstore/v1',
        method: ''
      }
    }
  }
...
test('listAddresses endpoint.getRequest() with valid payload gives a proper request', async () => {
  ...
  expect(request.url).toBe('aosfpc1dev-store.occa.us-ashburn-1.ocs.oc-test.com:443/ccstore/v1');
...
test('listAddresses with valid payload returns response populated with repository structure', async () => {
  ...
  expect(endpointResponse.url).toBe('aosfpc1dev-store.occa.us-ashburn-1.ocs.oc-test.com:443/ccstore/v1');
