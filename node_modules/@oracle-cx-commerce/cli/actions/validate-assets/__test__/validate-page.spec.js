...
describe('validate-assets:all  -  all the tests that will invoke validate-assets with different options', () => {
  ...
  test('validate-assets:validate-page-class-exists - check that page class is present', () => {
    const newPath = path.resolve(path.relative('.', __dirname), '..', 'page.js');
  ...
  test('validate-assets:check-properties-with-invalid-values - check proper messages shows up when properties contain invalid values', async () => {
    ...
    await expect(validateAssets(options.appName, options)).rejects.toThrow(
      `Asset validation failed, found 12 issue(s). Please see issue(s) listed above.`
    );
  });
  test('validate-assets:cross-validations - check proper messages shows up when doing cross validation for pages', async () => {
    const options = {
      ...coreCommerceReferenceStoreOptions,
      appDir: path.resolve(
        path.relative('.', __dirname),
