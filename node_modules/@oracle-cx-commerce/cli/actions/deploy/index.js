...
const deploy = async (appName, options) => {
    ...
    if (!options.localDevApp) {
      if (options.debugMode === true || options.debugMode === false) {
        ...
        if (cluster) {
          ...
          //we defer the setting of debug mode until after the deployment and the cluster
          //is assigned by the OCC server.
          deferPreviewDebug = true;
        }
      }
  ...
  try {
    ...
    // don't run publish
    if (activeStatus) {
      if (deferPreviewDebug && clusterId) {
        await redeploy(clusterId, {...options, live: false, force: true});
      }
