...
const deploy = async (appName, options) => {
  ...
  try {
    ...
    if (options.reset) {
      logger.info(___`Deploying application ${appName} with a reset to ${options.appServerAdmin}`);
    } else {
      logger.info(___`Deploying application ${appName} to ${options.appServerAdmin}`);
    }
