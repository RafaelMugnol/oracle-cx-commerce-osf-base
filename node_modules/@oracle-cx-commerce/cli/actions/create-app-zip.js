...
const {getListOfFilesToVerify, getSha256} = require('@oracle-cx-commerce/tools-util/files');
...
const NAME_VERSION_REGEX = /(?<name>@?[^@]+)@(?<version>[^@]+)/;

/**
 * Returns whether the package.json at the given path is the "root" one in the package
 * i.e., whether it defines the package's name and version, instead of just specifying the
 * "type" of some of the package's files.
 *
 * This isn't a perfect check for whether it's the root package.json, since technically lower
 * package.jsons could also define those fields, but it should filter out
 * {type: 'module'} package.jsons.
 *
 * @param {string} packageJsonPath an absolute path to the package.json to check
 * @returns {boolean} whether the package.json
 */
function isRootPackageJson(packageJsonPath) {
  return require(packageJsonPath).name && require(packageJsonPath).version;
}

/**
 * Returns whether a directory is the root of a Node.JS package
 * @param {string} fileDir A directory which should be checked to see if it's the root of a package.json
 * @returns {boolean} whether the given folder is the root or not.
 */
function isPackageRoot(fileDir) {
  return (
    fs.existsSync(fileDir) &&
    fs.readdirSync(fileDir).find(fPath => fPath.endsWith('package.json')) &&
    // This last check is to ensure that the package isn't just {"type": "module"} or the like
    // See https://github.com/sindresorhus/pkg-dir/issues/8
    isRootPackageJson(path.resolve(fileDir, 'package.json'))
  );
}
 ...
 * @param {string} filePath the absolute path to any file within the package
...
function getPackageRoot(filePath) {
  ...
  if (filePath.endsWith('package.json') && isRootPackageJson(filePath)) return path.dirname(filePath);
  // Otherwise recur up the file system until we reach the root of the file system or find the package json
  let recurFileDir = path.dirname(filePath);
  while (!isPackageRoot(recurFileDir)) {
    if (recurFileDir === FS_ROOT || recurFileDir === '.' || recurFileDir.endsWith('node_modules')) {
    ...
    recurFileDir = path.dirname(recurFileDir);
  }
 ...
 * @returns {Array<string>} dependencyPaths, an Array of string paths to the package resolved from the given package path
 */
function getDependencyPaths(dependencies, packagePath = process.cwd()) {
  ...
  dependencies.forEach(dependencyName => {
    try {
      ...
      // TODO revisit the above assumption when implementing features CCSF-17979, it may not hold after those go in
      logger.debug(error);
      logger.verbose(
 ...
 * Defaults to returning "true" a package is local, so we will definitely deploy it.
 *
...
/**
 * Based on a package's name, returns whether that package is local (symlinked
 * from node_modules by yarn/npm to another folder). Since local, hoisted packages
 * are available from the root's node_modules, we know if the package is local it'll
 * be available from the root's node_modules and symlink somewhere else.
 *
 * @param {string} name the name of the package
 * @returns {boolean} whether the package is local or not.
 */
function isLocalPackageName(name) {
  const nodeName = path.join('node_modules', name);

  return fs.existsSync(nodeName) && !fs.realpathSync(nodeName).endsWith(nodeName);
}
...
/** The path at which the yarn lockfile can be found */
const lockfilePath = path.resolve(process.cwd(), 'yarn.lock');
/** The parsed lockfile (object) in memory: {type: 'success|fail', object: {lockfile}} */
let parsedLockfile;

/**
 * Loads the lockfile if it exists (throws an error if it fails to parse or does not exist).
 * @returns {boolean} whether the lockfile is ready to be used or not
 * @throws if the lockfile could not be parsed/didn't exist.
 */
...
function getExternalDependencies(appDir) {
  ...
  try {
    ...
    externalNames.forEach(externalName => {
      try {
        ...
        let externalPackagePath = getDependencyPaths([externalName], appPackageJsonPath).pop();
        if (!externalPackagePath || !fs.existsSync(path.join(externalPackagePath, 'package.json'))) {
          ...
          if (childFile && !externalNames.has(packageName)) {
            ...
            externalPackagePath = getDependencyPaths([packageName], appPackageJsonPath).pop();
        ...
        // FIXME This will break if the yarn.lock is out of sync with the node_modules installation, CCSF-23325...
        // But it's that or be unclear which version to use if there are multiple versions...
        const {version} = JSON.parse(fs.readFileSync(path.join(externalPackagePath, 'package.json')));
  ...
  return {
    externalPkgs,
    externalDependencies: getYarnLockDependencies(externalPkgs)
  };
}

/**
 * Given a Set of dependencies, determines which of those dependencies
 ...
 * @param {Set} dependencies the Set of dependencies to evaluate
 * @returns {Object} an Object with two Sets:
 *   - oracleCommerceDependencies, a Set of dependency nameVersions whose exact
 *    versions are present in the oracle-cx-commerce registry
 *   - updatedOracleCommerceDependencies, a Set of nameVersions for packages
...
function getOracleCommerceDeps(dependencies, appDir) {
  if (!(dependencies instanceof Set)) {
    throw new Error(`getOracleCommerceDependencies expects a set, not a ${typeof dependencies}`);
  }
  ...
  try {
    ...
    const occMetadataLocation = getDependencyPaths(['@oracle-cx-commerce/occ-metadata'], appDir).pop();
    if (!occMetadataLocation) {
      throw new Error(
  ...
  dependencies.forEach(nameVersion => {
    try {
      const {name, version} = nameVersion.match(NAME_VERSION_REGEX).groups;
      ...
      if (
        isOracleCommerce(name) &&
        !require(path.join(getDependencyPaths([name], appDir).pop(), 'package.json')).private
      ) {
        oracleCommerceDependencies.add(nameVersion);
        ...
        if (manifestEntry.includes(version)) {
          ...
          updatedOracleCommerceDependencies.add(nameVersion);
        }
      }
      ...
      logger.error(___`Unable to load package.json of ${nameVersion}. Will not find its dependencies.`);
      logger.debug(error);
    }
  });
...
const UPDATE_CACHE_MESSAGE = ___`Try running "${'yarn cache clean && yarn --force'}" to force yarn to reinstall all dependencies. You may also need to add the "${'--update-checksums'}" and "${'--skip-integrity-check'}" flags, or delete the ${'node_modules'} folder *before* running the commands.`;
...
const getCachedFilePath = (name, version) => {
  ...
  try {
    ...
    logger.info(
      ___`Did not find ${name}@${version} at ${cachePackagePath}. Attempting to retrieve ${cacheName} from Yarn's global cache`
    );
    ...
    if (fs.existsSync(yarnCacheDir)) {
      ...
      throw new Error(___`Could not resolve yarn cache directory with output of 'yarn cache dir': ${yarnCacheDir}`);
    }
  } catch (e) {
    // Swallow rebuilding cache error to show the more relevant problem: needing to rebuild cache in the first place
    logger.debug(___`Rebuilding .yarncache from global yarn cache failed`);
    logger.debug(e);
  }
  throw new Error(
 ...
 */
const getDependencyPathToSave = (name, version) => {
  // If the package is local, return its path inside packages/ (found by getting the real-path from node_modules)
  if (isLocalPackageName(name)) {
    return fs.realpathSync(path.join('node_modules', name));
  }

  //Otherwise return the path to the cached .tgz file
  return getCachedFilePath(name, version);
};

/**
 * Returns the given set with the packages in the dependencyMap (and their dependencies) added to it,
 * based on dependency listings in the shared yarn.lock
 *
 * @param {Object} dependencyMap A map from names of dependencies to their version-strings (e.g. {'fs-extra': '^8.1.0'})
 * @param {Set<string>} allDependencies a set of dependencies to which the given dependencies and their dependencies should be added.
 * @returns {Set<string>} the set with the given dependencies (their name-versions) and their dependencies added, e.g. Set(['fs-extra@8.1.3'])
 */
function getYarnLockDependencies(dependencyMap = {}, allDependencies = new Set()) {
  try {
    if (!ensureLockfile())
      throw new Error(___`yarn.lock could not be parsed. Please delete it and run ${'yarn install'}`);

    const entries = dependencyMap instanceof Map ? [...dependencyMap.entries()] : Object.entries(dependencyMap);

    entries.forEach(([name, versionString]) => {
      const nameVersion = `${name}@${versionString}`;
      // Get the entry in the yarn.lock or, failing that, its package.json
      const lockEntry = parsedLockfile.object[nameVersion];
      if (lockEntry) {
        // Get the exact version used (notVersionString), return if we've already seen it, otherwise recur
        const {version} = lockEntry;
        if (allDependencies.has(`${name}@${version}`)) return;
        allDependencies.add(`${name}@${version}`);
        getYarnLockDependencies({...lockEntry.dependencies, ...lockEntry.optionalDependencies}, allDependencies);
      } else {
        // Get the exact version used (notVersionString), return if we've already seen it, otherwise recur
        const dependencyPath = getDependencyPaths([name], process.cwd()).pop();
        // console.assert(isLocal(dependencyPath), 'FAILED ASSERTION'); // won't hold for Marcus
        const {version, dependencies, optionalDependencies} = JSON.parse(
          fs.readFileSync(path.join(dependencyPath, 'package.json'))
        );
        if (allDependencies.has(`${name}@${version}`)) return;
        allDependencies.add(`${name}@${version}`);
        // If shipping up devDependencies, add to below
        getYarnLockDependencies({...dependencies, ...optionalDependencies}, allDependencies);
      }
    });
 ...
 */
module.exports = async (appName, options, archiveCallBack) => {
  // TODO DEPRECATED if they opt into the old method, use that until we remove it
  if (/true/i.test(process.env.OCC_OLD_DEPLOYMENT) && fs.existsSync(path.join(__dirname, 'create-app-zip.old.js'))) {
    logger.warn(___`Found 'OCC_OLD_DEPLOYMENT' environment variable. Using the old, *deprecated* deployment method.`);

    return require('./create-app-zip.old')(appName, options, archiveCallBack);
  }
  // Log the checksums for various important files
  try {
    const fileList = getListOfFilesToVerify(options.appDir);
    logger.verbose(`Checksums: ${inspect(await getSha256(fileList))}`);
  } catch (e) {
    logger.verbose(`Could not compute checksum for application. Continuing, but see error below.`);
    logger.verbose(e);
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const allDependencies = getYarnLockDependencies(
      JSON.parse(fs.readFileSync(path.resolve(appDir, 'package.json'))).dependencies
    );
    ...
    if (fs.existsSync(workspaceJsonPath) && isWorkspace(require(workspaceJsonPath))) {
      getYarnLockDependencies(JSON.parse(fs.readFileSync(workspaceJsonPath)).dependencies, allDependencies);
    }
    ...
    if (options.deployType === 'lite') {
      process.env.OCC_APP_DIR = appDir; // Set the app dir so rollup config can grab it when getting externals list
    ...
    if (!fullDeployment) {
      allDependencies.forEach(nameVersion => {
        const {name} = nameVersion.match(NAME_VERSION_REGEX).groups;
        if (isLocal(getDependencyPaths([name]).pop()) && !isOracleCommerce(name || '')) {
          dependenciesToSave.add(nameVersion);
        }
      });
    }
    ...
    for (const nameVersion of dependenciesToSave) {
      const {name} = nameVersion.match(NAME_VERSION_REGEX).groups;
      // Remove dependencies from the list to add to the zip if they're available from the production registry
      // (in oracleCommerceDependencies) or this is a localDevApp and the dependency is from a non-node_modules source
      if (oracleCommerceDependencies.has(nameVersion) || (options.localDevApp && !isLocalPackageName(name))) {
        dependenciesToSave.delete(nameVersion);
        dependenciesSkipped++;
      }
    }
    ...
    dependenciesToSave.forEach(nameVersion => {
      const {name, version} = nameVersion.match(NAME_VERSION_REGEX).groups;
      ...
      const depPath = getDependencyPathToSave(name, version);
      if (fs.existsSync(depPath)) {
        ...
        throw new Error(
          ___`Could not find file or directory '${depPath}' for dependency '${nameVersion}'. A new 'yarn install' may be required`
        );
      }
    });
