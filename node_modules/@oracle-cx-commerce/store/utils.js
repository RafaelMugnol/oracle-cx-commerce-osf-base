...
import {measureTime} from '@oracle-cx-commerce/utils/perf';
...
export const createSagaErrorHandler = ({store, wapi}) => {
  ...
  return error => {
    ...
    console.warn(`[API] Error "${error.message}" has been logged with the OCC server`);
  };
};
  ...
  if (__ENABLE_USER_TIMING_API__) {
    ...
    const getSagaLabel = (effect, effectId) => {
      ...
      if (effect && effect.root) {
        ...
        label = 'Occ-Saga-Resolved-cps';
        if (data.fn && data.fn.name > 0) {
          label = `${label}-${data.fn.name[0]}`;
        }
      } else if (is.effect(data) && data.type === effectTypes.FORK) {
        if (!data.detached) {
          label = 'Occ-Saga-Resolved-fork';
        } else {
          label = 'Occ-Saga-Resolved-spawn';
        }
