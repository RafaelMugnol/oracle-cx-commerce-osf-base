...
const setForwardedHeaders = (req, res) => {
  ...
  if (!req.get('X-Forwarded-Proto')) {
    res.locals['X-Forwarded-Proto'] = req.get('x-scheme') || req.protocol;
  }

  if (!req.get('Forwarded')) {
    const byId = '_Oracle_OCX_OSF';
    const host = req.get('Host');
    const proto = req.get('x-scheme') || req.protocol;
...
const setBrowserGlobals = (req, res) => {
  ...
  res.locals.location = new URL(`${protocol}://${req.get('Host')}${req.originalUrl}`);
};
